...
function timeSpanToString(startDate, endDate) {
  ...
  const min = (endDate.getMinutes() - startDate.getMinutes()).toString().padStart(2, 0);
  const sec = (endDate.getSeconds() - startDate.getSeconds()).toString().padStart(2, 0);
  const mlSec = (endDate.getMilliseconds() - startDate.getMilliseconds()).toString().padStart(3, 0);
...
function angleBetweenClockHands(date) {
  ...
  return Math.abs((((0.5 * (60 * hours - 11 * min)) > 180 ? 360 - (0.5 * (60 * hours - 11 * min))
    : (0.5 * (60 * hours - 11 * min))) * (Math.PI / 180)));
}
